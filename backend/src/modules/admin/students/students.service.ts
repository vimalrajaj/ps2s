...
import type { CreateStudent, UpdateStudent, StudentQuery } from './students.schemas.js';
...
export class StudentsService {
  ...
  static async getStudents(query: StudentQuery) {
  ...
  static async updateStudent(id: string, data: UpdateStudent) {
    const { data: student, error } = await supabase
      .from('students')
      .update(data)
      .eq('id', id)
  ...
  static async getStudentStats() {
    ...
    const semesterCounts = stats?.reduce((acc, s) => {
      const semester = (s.classes as any)?.semester || 0;
      acc[`Semester ${semester}`] = (acc[`Semester ${semester}`] || 0) + 1;
      return acc;
    }, {} as Record<string, number>) || {};
    const departmentCounts = stats?.reduce((acc, s) => {
      const deptName = (s.classes as any)?.departments?.name || 'Unknown';
