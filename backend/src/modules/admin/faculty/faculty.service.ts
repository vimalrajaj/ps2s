...
import type { CreateFaculty, UpdateFaculty, FacultyQuery } from './faculty.schemas.js';

export class FacultyService {
  static async createFaculty(data: CreateFaculty) {
    // Hash password
  ...
  static async getFaculty(query: FacultyQuery) {
  ...
  static async updateFaculty(id: string, data: UpdateFaculty) {
    const { data: faculty, error } = await supabase
      .from('faculty')
      .update(data)
      .eq('id', id)
  ...
  static async getFacultyStats() {
    ...
    const departmentCounts = stats?.reduce((acc, f) => {
      const deptName = (f.departments as any)?.name || 'Unknown';
